...
func _ready():
	...
	initial_position = skeleton_enemy.global_position
...
func enemy_attack(delta : float):
	...
	if abs(position.x - parent.position.x) > 45 and !is_attacking:
	...
	if !on_cooldown and is_attacking == false and abs(position.x - parent.position.x) < 45:
		...
		if abs(position.x - parent.position.x) < 45:
...
func _is_dead(delta: float):
	if health_Points < 0 and !is_dead:
		...
		if skeleton_enemy:
			skeleton_enemy.visible = false
			aggroArea.monitoring = false
			for child in skeleton_enemy.get_children():
